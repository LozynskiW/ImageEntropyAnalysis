...
"""
Algorithms that search for target on image based on some principle. Result of each function are target coordinates, which
based on principle may be mean value on (x,y) coordinates taken from each pixel on image or (x,y) of center of area where
luminance is highest etc.
Img that is given for algorithms is assumed to only contain target pixels. If this condition is not met result of target
detection may be inaccurate or worse...
"""


class MeanShift:

    def __init__(self, verbose_mode=False, show_image_after_processing=False, fast_search=False):
        self.__verbose_mode = verbose_mode
        self.__show_image_after_processing = show_image_after_processing
        self.__fast_search = fast_search
    ...
    def search_for_target(self, segmented_img):
        ...
        if self.__show_image_after_processing:
